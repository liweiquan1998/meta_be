...
@web_try()
@sxtimeit
def create_background(item: schemas.BackgroundCreate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def update_background(background_id: int, update_item: schemas.BackgroundUpdate, db: Session = Depends(get_db)):
