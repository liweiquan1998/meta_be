...
@web_try()
@sxtimeit
def add_live_account(item: schemas.LiveAccountCreate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def get_live_account_once(item_id: int, db: Session = Depends(get_db), ):
...
@web_try()
@sxtimeit
def get_live_account_once(creator_id: int, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def update_live_account(item_id: int, update_item: schemas.LiveAccountUpdate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def delete_live_account(item_id: int, db: Session = Depends(get_db)):
