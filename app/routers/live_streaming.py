...
@web_try()
@sxtimeit
def add_live_streaming(item: schemas.LiveStreamingCreate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def get_live_streaming_once(item_id: int, db: Session = Depends(get_db), ):
...
@web_try()
@sxtimeit
def update_live_streaming(item_id: int, update_item: schemas.LiveStreamingUpdate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def delete_live_streaming(item_id: int, db: Session = Depends(get_db)):
