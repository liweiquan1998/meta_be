...
@web_try()
@sxtimeit
def add_store(item: schemas.StoreCreate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def delete_store(item_id: int, db: Session = Depends(get_db), ):
...
@web_try()
@sxtimeit
def update_store(item_id: int, update_item: schemas.StoreUpdate, db: Session = Depends(get_db)):
...
@web_try()
@sxtimeit
def get_store_once(item_id: int, db: Session = Depends(get_db), ):
...
@web_try()
@sxtimeit
def get_stores(get_item: schemas.StoreGet = Depends(), params: Params = Depends(), db: Session = Depends(get_db), ):
