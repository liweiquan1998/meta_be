...
@web_try()
@sxtimeit
def get_shelves_once(item_id: int, db: Session = Depends(get_db), user=Depends(check_user) ):
...
@web_try()
@sxtimeit
def update_shelves(item_id: int, update_item: schemas.ShelvesUpdate, db: Session = Depends(get_db), user=Depends(check_user)):
